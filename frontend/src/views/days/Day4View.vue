<template>
  <day-view :day-number="4" :visible="visible" @completed="onCompleted">
    <div class="day-specific-content">
      <p>勇者的第四天，魔王即将到来，你需要做最后的准备。</p>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const choices = [
      { text: '寻找传说中的神器', value: 'artifact' },
      { text: '与伙伴们制定战略', value: 'strategy' },
      { text: '休息恢复全部状态', value: 'rest' }
    ]
    ...
    const onCompleted = async () => {
      if (selectedChoice.value !== null) {
        try {
          // 将选择发送到后端
          await axios.post('/api/game/day4', {
            choice: choices[selectedChoice.value].value
          })
        } catch (error) {
          console.error('提交第四天选择失败:', error)
        }
      }
